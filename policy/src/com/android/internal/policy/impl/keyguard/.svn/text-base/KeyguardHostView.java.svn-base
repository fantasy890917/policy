...
/** [HQ_WALLPAPER_CATEGORY] xiehongbin 2013103 start*/
...
/** [HQ_WALLPAPER_CATEGORY] xiehongbin 20131031  end*/
...
public class KeyguardHostView extends KeyguardViewBase {
    ...
    /**[HQ_WALLPAPER_CATEGORY] xiehongbin 20131031 start **/
	private static Context mLauncherContext=null;
	private LockScreenBackGroundLoader mLockScreenBackGroundLoader;
	private static final String LOCK_SCREEN_BACKGROUND_ACTION="android.intent.action.LOCKSCREENWALLPAPER";
	private int LOCK_RES_ID=-1;
	private int IS_FROM_GALLERY= 0;//wallpaper not come from gallery ,1 is   
	private Resources mResources;
	public static final String KEY_LOCKSCREEN_GALLERY = "temp_lockscreen_wallpaper.jpg";
	/**[HQ_WALLPAPER_CATEGORY] xiehongbin 20131031  end **/
...
/** [HQ_WALLPAPER_CATEGORY] xiehongbin 20131031  start*/
	if(FeatureOption.HQ_WALLPAPER_CATEGORY) {
		getLockScreenRes(context);
		mLockScreenBackGroundLoader = new LockScreenBackGroundLoader(context);
		IntentFilter filter = new IntentFilter();
	         filter.addAction(LOCK_SCREEN_BACKGROUND_ACTION);
	         context.registerReceiver(mLockScreenBackGroundLoader, filter);
	}
/** [HQ_WALLPAPER_CATEGORY] xiehongbin 20131031  end*/
	...
	/** [HQ_WALLPAPER_CATEGORY] xiehongbin 20131031  start*/
		if(FeatureOption.HQ_WALLPAPER_CATEGORY) {
		int isWallpaperFromGallery= Settings.Secure.getInt(mContext.getContentResolver(),
													  Settings.Secure.LOCKSCREEN_WALLPAPER_GALLERY, -1);
		if(isWallpaperFromGallery==1){
			setBackgroundDrawable(getGalleryWallpaper(KEY_LOCKSCREEN_GALLERY,mLauncherContext));
			
		}else{
			int thisId= Settings.Secure.getInt(mContext.getContentResolver(),
													  Settings.Secure.LOCK_SCREEN_ID, -1);
			if(thisId != -1) {
				setBackgroundDrawable(this.mResources.getDrawable(thisId));
			}
		}
   ...
   /** [HQ_WALLPAPER_CATEGORY] xiehongbin 20131031  start*/
	private class LockScreenBackGroundLoader extends BroadcastReceiver{
	    public LockScreenBackGroundLoader(Context context) {
	    }

    	@Override
	    public void onReceive(final Context context, final Intent intent) {
	        final String action = intent.getAction();
	        if (LOCK_SCREEN_BACKGROUND_ACTION.equals(action)) {
	            LOCK_RES_ID = intent.getIntExtra("xerrard",-1);
		    IS_FROM_GALLERY = intent.getIntExtra("isgallery",-1);
		    Log.d(TAG+"shiguibiao","[onReceive] LOCK_RES_ID = "+LOCK_RES_ID);
			//save id 
			Settings.Secure.putInt(mContext.getContentResolver(), Settings.Secure.LOCK_SCREEN_ID, LOCK_RES_ID);
			Settings.Secure.putInt(mContext.getContentResolver(), Settings.Secure.LOCKSCREEN_WALLPAPER_GALLERY, IS_FROM_GALLERY);
		    }
	    }
    }

   private void getLockScreenRes(Context context){ 
		  try {
		   mLauncherContext=context.createPackageContext("com.android.launcher",
				  Context.CONTEXT_INCLUDE_CODE | Context.CONTEXT_IGNORE_SECURITY);
		  }catch(NameNotFoundException e){
		  		Log.d(TAG+"shiguibiao","cannot get name");
		  	}
		  if (mLauncherContext!= null) {
			  this.mResources=mLauncherContext.getResources();
		  }
	}

	public Drawable getGalleryWallpaper(String fileName,Context c){
		return bitmapToDrawable(getBitmapfromfile(fileName,c));
	}
	public  Drawable bitmapToDrawable(Bitmap bitmap) {
		Drawable drawable = new BitmapDrawable(bitmap);
		return drawable;
	}
   	public  Bitmap getBitmapfromfile(String fileName,Context c){
		Bitmap bm = null;
