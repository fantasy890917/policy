...
public class KeyguardViewMediator {
                ...
                case NETWORK_LOCKED:
                    synchronized (this) {
                        ...
                        if (IccCardConstants.State.NETWORK_LOCKED == simState
                            && 0 == mUpdateMonitor.getSimMeLeftRetryCount(PhoneConstants.GEMINI_SIM_1)) {
                        ...
                        /// @}
                        if ((IccCardConstants.State.PIN_REQUIRED == simState && mUpdateMonitor.getPINDismissFlag(PhoneConstants.GEMINI_SIM_1, KeyguardUpdateMonitor.SimLockType.SIM_LOCK_PIN))
                            || (IccCardConstants.State.PUK_REQUIRED == simState && mUpdateMonitor.getPINDismissFlag(PhoneConstants.GEMINI_SIM_1, KeyguardUpdateMonitor.SimLockType.SIM_LOCK_PUK))
                            || (IccCardConstants.State.NETWORK_LOCKED == simState && mUpdateMonitor.getPINDismissFlag(PhoneConstants.GEMINI_SIM_1, KeyguardUpdateMonitor.SimLockType.SIM_LOCK_ME))) {
                            KeyguardUtils.xlogD(TAG, "We have dismissed locks, so, we should reset the dismiss flags");
                            mUpdateMonitor.setPINDismiss(PhoneConstants.GEMINI_SIM_1, KeyguardUpdateMonitor.SimLockType.SIM_LOCK_PIN, false);
                            mUpdateMonitor.setPINDismiss(PhoneConstants.GEMINI_SIM_1, KeyguardUpdateMonitor.SimLockType.SIM_LOCK_PUK, false);
                            mUpdateMonitor.setPINDismiss(PhoneConstants.GEMINI_SIM_1, KeyguardUpdateMonitor.SimLockType.SIM_LOCK_ME, false);
                        }
                        if (!isShowing()) {
    ...
    private void doKeyguardLocked(Bundle options) {
        boolean isSimSecure = KeyguardUpdateMonitor.getInstance(mContext).isSimPinSecure();
        ///M: if another app is disabling us (except Sim Secure), then don't show
        if ((!mExternallyEnabled && !isSimSecure)|| KeyguardUpdateMonitor.isAlarmBoot()) {
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            } else if (IPO_BOOT.equals(action)) {
                mHideNavBar = false;
                mIsIPOBoot = true;
                Log.w(TAG, "IPO_BOOT: " + action + "  alarmBoot: " + KeyguardUpdateMonitor.isAlarmBoot());
                if (KeyguardUpdateMonitor.isAlarmBoot()) {
                    hideLocked();
                }
                ...
                case RESHOW_ANYWHERE:
                    ...
                    postDelayed(new Runnable(){
                        public void run(){
                            ...
                            if(!mLockPatternUtils.isLockScreenDisabled() || isSecure()) {
                                handleShow(null);
                            }
                        }
                    }, 2000);
  ...
  /// M: For DM lock feature to update keyguard
    ...
    private void handleDMKeyguardUpdate(boolean lock) {
        ...
        sendReLayoutMsg(lock, 0);
    }
    
    /// M: send relayout msg to self for dm or power off alarm
    private void sendReLayoutMsg(boolean lock, long delayTime) {
