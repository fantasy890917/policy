...
public class KeyguardMultiUserSelectorView extends FrameLayout implements View.OnClickListener {
    ...
    private boolean hasAppsAccessingSD(){
        try {
            ...
            int stUsers[] = mountService.getStorageUsers("/storage/sdcard1");
            if (stUsers != null && stUsers.length > 0) {
                return true;
            }
            int otgUsers[] = mountService.getStorageUsers("/mnt/usbotg");
            if (otgUsers != null && otgUsers.length > 0) {
                return true;
            }
            return false;
    ...
    public void onClick(View v) {
        ...
        if (avatar.isClickable()) { // catch race conditions
            ...
            /// M: sdcard&otg only for owner @{
                ...
                bdl.setMessage(com.android.internal.R.string.sd_accessing_swtich_user_message);
                bdl.setTitle(com.android.internal.R.string.sd_accessing_swtich_user_title);
