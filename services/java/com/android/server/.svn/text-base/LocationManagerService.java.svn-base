...
public class LocationManagerService extends ILocationManager.Stub implements Runnable {
    ...
    // mapping from provider name to all its UpdateRecords
    private final HashMap<String, MomsRequestRecord> mRequestPending =
            new HashMap<String, MomsRequestRecord>();
    //MTK end
    ...
    public void requestLocationUpdates(LocationRequest request, ILocationListener listener,
        ...
        if(FeatureOption.MTK_MOBILE_MANAGEMENT == true) {
            Log.d(TAG, "moms requestLocationUpdates pkg:" + packageName + " uid:" + uid + " receiver:" + recevier);
            if(!mRequestPending.containsKey(Integer.toString(uid))) {
                synchronized(mRequestPending) {
                    MomsRequestRecord mRequestCheckItem = new MomsRequestRecord(sanitizedRequest, recevier, pid, uid, packageName);
                    mRequestPending.put(Integer.toString(uid),mRequestCheckItem);
                }
            } else {
                Log.d(TAG, "requestLocationUpdates containKey "+ uid);
            }
            Log.d(TAG, "requestLocationUpdates mRequestPending size:"+ mRequestPending.size());
            try {
                ...
                mMobileManager.checkPermissionAsync(SubPermissions.ACCESS_LOCATION,uid, new PermissionCallback());
            } catch (RemoteException e) {
                Log.e(TAG, "chcek permission Asyn failed :" +  SubPermissions.ACCESS_LOCATION + " package:" + packageName);
            } catch (Exception e) {
    ...
    private void removeUpdatesLocked(Receiver receiver) {
        ...
        if(FeatureOption.MTK_MOBILE_MANAGEMENT == true) {
            final int uid = Binder.getCallingUid();
            Log.d(TAG, "removeUpdatesLocked remove Pending uid:" + receiver.mUid);
            if(mRequestPending.containsKey(Integer.toString(uid))) {  //recover record and remove pending item
                mRequestPending.remove(Integer.toString(uid));
            }
        }
    ...
    //
    private class PermissionCallback extends IRequestedPermissionCallback.Stub {
        @Override
        public void onPermissionCheckResult(String permissionName, int uid, int result) {
            Log.d(TAG, "onPermissionCheckResultCB pkg:" + permissionName + " uid:" + uid + " result:" + result);
            synchronized(mRequestPending) {
                switch (result) {
                    case PackageManager.PERMISSION_GRANTED:
                        {
                            MomsRequestRecord mMomsRequest = mRequestPending.get(Integer.toString(uid));

                            if(mMomsRequest == null) {
                                Log.d(TAG, "mRequestPending size:" + mRequestPending.size() + " break");
                                break;
                            }

                            try {
                                synchronized (mLock) {
                                    requestLocationUpdatesLocked(mMomsRequest.mRequest, mMomsRequest.mRecevier, mMomsRequest.mpid, mMomsRequest.muid, mMomsRequest.mPackageName);
                                }
                            } finally {
                                mRequestPending.remove(Integer.toString(uid));
                            }
                    ...
                    case PackageManager.PERMISSION_USER_DENIED:
                        if(mRequestPending.containsKey(Integer.toString(uid))) {
                            Log.d(TAG, "DENIED remove Pending uid:" + uid);
                            MomsRequestRecord mMomsRequest = mRequestPending.get(Integer.toString(uid));
                            if (mReceivers.remove(mMomsRequest.mRecevier.mKey) != null && mMomsRequest.mRecevier.isListener()) {
                                mMomsRequest.mRecevier.getListener().asBinder().unlinkToDeath(mMomsRequest.mRecevier, 0);
                                synchronized (mMomsRequest.mRecevier) {
                                    if (mMomsRequest.mRecevier.mPendingBroadcasts > 0) {
                                        decrementPendingBroadcasts();
                                        mMomsRequest.mRecevier.mPendingBroadcasts = 0;
                                    }
                                }
                            }
                            mRequestPending.remove(Integer.toString(uid));
                        ...
                        break;
                    default:
                        Log.d(TAG, "onPermissionCheckResultCB result error");
                        break;
                }
                Log.d(TAG, "onPermissionCheckResultCB mRequestPending check size:"+ mRequestPending.size());
            }
        }
    }

    private class MomsRequestRecord {
        ...
        /**
            * Note: must be constructed with lock held.
            */
        MomsRequestRecord(LocationRequest request, Receiver receiver,
            int pid, int uid, String packageName) {
            mRequest = request;
            mRecevier = receiver;
            mpid = pid;
