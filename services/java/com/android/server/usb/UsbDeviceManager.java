    ...
    private final class UsbHandler extends Handler {
        ...
        public UsbHandler(Looper looper) {
            ...
            try {
                ...
                if (DEBUG) SXlog.d(TAG, "persist.sys.port_index:" + value);
                if (value.length() > 0) {
                    int port_index = Integer.parseInt(value);
                    if (port_index >= 1 && port_index <= 4) {
                        mAcmPortNum = port_index;

                        writeFile(ACM_PORT_INDEX_PATH, String.valueOf(mAcmPortNum));
        ...
        private void setEnabledFunctions(String functions, boolean makeDefault) {
            ...
            if (functions != null && makeDefault && !needsOemUsbOverride() && !mIsUsbBicrEvo) {
                ...
                if (mAcmEnabled && !mSettingUsbCharging && !mSettingUsbBicr) {
                    if (mAcmPortNum > 0) {
                        SXlog.d(TAG, "Set the specified port num" + mAcmPortNum);
                        writeFile(ACM_PORT_INDEX_PATH, String.valueOf(mAcmPortNum));
                    }
                    functions = addFunction(functions, UsbManager.USB_FUNCTION_ACM);
                } else {
        ...
        private void updateUsbState() {
			...
			SXlog.w(TAG, "sys.usb.mtpConnect = " + SystemProperties.get("sys.usb.mtpConnect"));
			//ALPS00606302
        }
            ...
            switch (msg.what) {
                ...
                case MSG_ENABLE_ACM:
                    ...
                    if (portNum >= 1 && portNum <= 4)
                        ...
                        mAcmPortNum = 0;
                    SXlog.d(TAG, "mAcmPortNum=" + mAcmPortNum);
                    setAcmEnabled(mAcmPortNum > 0);
                    break;
