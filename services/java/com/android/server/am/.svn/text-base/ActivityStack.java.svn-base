...
final class ActivityStack {
    ...
    private boolean mIsMultiLauncher = false; /// M: add for avoid screen capture for launcher
...
//                    Slog.e(TAG,"No animation. Resume the top activity");
        ...
        while (i < N) {
            ...
            if (r.userId == userId) {
                ...
                if (null != mStackListener){
                    mStackListener.dumpStack(mHistory);
                }
                /// @}
    ...
    final int startActivityLocked(IApplicationThread caller,
        ...
        /// M: add for avoid screen capture, if there are multiple launchers, let it capture the launcher snapshot.
        if (intent.getCategories() != null && intent.getComponent() != null && !mIsMultiLauncher) {
            if (intent.getComponent().flattenToShortString().equals("android/com.android.internal.app.ResolverActivity") &&
                    intent.getCategories().contains(Intent.CATEGORY_HOME)) {
                mIsMultiLauncher = true;
            }
        }
