...
public class ActiveServices {
    ...
    private boolean checkServiceRestartMemoryLocked(ServiceRecord sr) {
        ...
        if (!isRestart) {
            ...
            if (avgFreeMem > mAm.mProcessList.getMemLevel(ProcessList.HIDDEN_APP_MIN_ADJ) ||
                avgFreeMem > mAm.mProcessList.getMemLevel(ProcessList.BACKUP_APP_ADJ) + sr.memUsage) {
                isRestart = true;
            }
            else {
                long delayTime = 0;
