...
import com.android.internal.R;//[HQ_REBOOT_LONGPRESS_POWER]
...
public final class ShutdownThread extends Thread {
    ...
    static void shutdownInner(final Context context, boolean confirm) {
		...
		/**[HQ_REBOOT_LONGPRESS_POWER--start]**/		
		if (mRebootSafeMode ==false && mReboot == true){		
                    sConfirmDialog = new AlertDialog.Builder(context)
                        .setTitle(R.string.hq_global_action_power_reboot_title)
                        .setMessage(R.string.hq_global_action_power_reboot_confirm)
                    ...
                    .create();
		}else{/**[HQ_REBOOT_LONGPRESS_POWER--end]**/	
                sConfirmDialog = new AlertDialog.Builder(context)
    ...
    private static void beginShutdownSequence(Context context) {
		...
		if(FeatureOption.HQ_SHUTANIMATION) {
			mShutOffAnimation = true;
		}
        synchronized (mEnableAnimatingSync) {
...
//                sInstance.mPowerManager.setBacklightBrightness(PowerManager.BRIGHTNESS_DIM);
				...
				if (false == FeatureOption.HQ_SHUTANIM_PLAY_END) {
    ...
    public void running() {
        ...
        if (mShutdownFlow == IPO_SHUTDOWN_FLOW) {
            ...
            if (pd != null) {
                ...
                try {
					if (FeatureOption.HQ_SHUTANIM_PLAY_END) {
						String bootanimFinished = SystemProperties.get("bootanim.finished");
						String bootanimLoop = SystemProperties.get("bootanim.loop");
						boolean retFin = (bootanimFinished != null && bootanimFinished.equals("1")) ? true : false;
						...
						while (true) {
							...
							bootanimFinished = SystemProperties.get("bootanim.finished");
							bootanimLoop = SystemProperties.get("bootanim.loop");
							retFin = (bootanimFinished != null && bootanimFinished.equals("1")) ? true : false;
    ...
    public static void rebootOrShutdown(boolean reboot, String reason) {
        if (reboot) {
			...
			if(FeatureOption.HQ_SHUTANIM_PLAY_END_NEW){
	       		delayForPlayAnimation();
			}
	        else if ( (reason != null) && reason.equals("recovery") ) {
                delayForPlayAnimation();
            }
            try {
