...
public class Tethering extends INetworkManagementEventObserver.Stub {
    ...
    public void interfaceStatusChanged(String iface, boolean up) {
        ...
        synchronized (mPublicSync) {
            ...
            if (up) {
                ...
                /** M: Whole chip reset : ignore wifi as well */
            ...
            if (isWifi(iface)) {
                ...
                if (service != null && wifiService != null) {
                    new Thread() {
                        public void run() {
                            try {
                                boolean startRequest = service.getStartRequest();
                                Log.d(TAG, "restartAccessPoint getStartRequest:" + startRequest);
                                if (!startRequest) {
                                    WifiConfiguration config = wifiService.getWifiApConfiguration();
                                    wifiService.setWifiApEnabled(null, false);
                                    wifiService.setWifiApEnabled(config, true);
                                }
                            } catch (Exception e) {
    ...
    public void interfaceRemoved(String iface) {
        ...
        synchronized (mPublicSync) {
            ...
            /** M: Whole chip reset @{ */
            if (isWifi(iface)) {
                Xlog.d(TAG, "Should not get here!!!");
                IBinder b = ServiceManager.getService(Context.NETWORKMANAGEMENT_SERVICE);
    ...
    private void showUsbInternetNotification(int icon, String type) {
        Log.d(TAG, "showUsbInternetNotification icon:"+icon+", type:"+type);
    ...
    private void showTetheredNotification(int icon, String type) {
        Log.d(TAG, "showTetheredNotification icon:"+icon+", type:"+type);
        synchronized (Tethering.this.mNotificationSync) {
             NotificationManager notificationManager =
                     (NotificationManager)mContext.getSystemService(Context.NOTIFICATION_SERVICE);
             if (notificationManager == null) {
    ...
    private void updateTetheredNotification() {
        synchronized (Tethering.this.mNotificationSync) {
              boolean wifiTethered = false;
              Log.d(TAG, "updateTetheredNotification");
              Set ifaces = mIfaces.keySet();
              for (Object iface : ifaces) {
                  TetherInterfaceSM sm = mIfaces.get(iface);
    ...
    private boolean hasDhcpv6PD(String iface) {
        if( FeatureOption.MTK_IPV6_TETHER_PD_MODE )
        {
            ...
            String value = SystemProperties.get(propertyName);
            if ( value == null || value.length() == 0){
                Log.d(TAG, "This is No Dhcpv6PD prefix!");
                return false;
            } else {
                Log.d(TAG, "This is Dhcpv6PD prefix: " + value);
                return true;
            }
        }
        else
            ...
            private void cleanupUpstreamIpv6() {
                if (mMyUpstreamIfaceNameIpv6 != null) {
                    ...
                    try {
                            ...
                            Xlog.d(TAG, "[ISM_Tethered] cleanupUpstream clearRouteIpv6("+ mIfaceName + ", " + mMyUpstreamIfaceNameIpv6 + ")");
                    } catch (Exception e) {
                                    ...
                                    if (ifcg != null && ifcg.isActive()) {
                                        ...
                                        if (FeatureOption.MTK_TETHERINGIPV6_SUPPORT) {
                                            if (smName == null || MASTERSM_IPV4.equals(smName)) {
                                                ...
                                                Xlog.d(TAG, "[ISM_Tethered] " + "clearRouteIpv6(" + mIfaceName + ", " + newUpstreamIfaceName + ")");
                                                mNMService.clearRouteIpv6(mIfaceName, newUpstreamIfaceName);
                                            }
                                        } else { /** @} */
                                            mNMService.disableNat(mIfaceName, newUpstreamIfaceName);
    ...
    class TetherMasterSM extends StateMachine {
        ...
        class TetherMasterUtilState extends State {
            ...
            protected void chooseUpstreamType(boolean tryCell) {
                ...
                synchronized (mPublicSync) {
                                        ...
                                        upType = ConnectivityManager.TYPE_NONE;
                                            ...
                                            try {
                                                ...
                                                if( ifacePD != null && !hasDhcpv6PD(ifacePD)){
                                                    Xlog.d(TAG, "we have no dhcp ipv6 PD address, iface:" + ifacePD);
                                                    upType = ConnectivityManager.TYPE_NONE;
                                                } else {
                                                    ifacePD = null;
                                                }
                    ...
                    if ( FeatureOption.MTK_TETHERINGIPV6_SUPPORT && mName.equals(MASTERSM_IPV6) && FeatureOption.MTK_IPV6_TETHER_PD_MODE &&
                        mIpv6FeatureEnable )
                    {
                        Xlog.d(TAG, "mPreviousDhcpv6PDIface:" + mPreviousDhcpv6PDIface + ",ifacePD:"+ ifacePD + ",upType:" + upType);
            ...
            protected void runDhcpv6PDSequence(String iface) {
                Xlog.d(TAG, "runDhcpv6PDSequence:" + iface);
                if( mDhcpv6PDThread == null )
                {
                    Xlog.d(TAG, "mDhcpv6PDThread is null, creating thread");
                ...
                {
                    Xlog.d(TAG, "mDhcpv6PDThread is not null");
                }
            }
            ...
            private class MyRunDhcpv6PDSequence implements Runnable{
                ...
                public void run() {
                        ...
                        Xlog.d(TAG, "runDhcpv6PD:" + mIface);
                        if (!NetworkUtils.runDhcpv6PD(mIface, dhcpInfoInternal)) {
                       ...
                       mDhcpv6PDThread = null;
                       Xlog.d(TAG, "Finish runDhcpv6PD:" + mIface);
                    }
            }
            /** @} */
        }
        ...
        protected void stopDhcpv6PDSequence() {
            Xlog.d(TAG, "stopDhcpv6PD:" + mPreviousDhcpv6PDIface);
            if( mPreviousDhcpv6PDIface != null )
            {
